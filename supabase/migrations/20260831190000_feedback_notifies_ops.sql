...
--
-- Автора не исключаем, хотя соблазн есть: «он и так знает, что написал».
-- Первым, кто пишет в поддержку, оказывается сам владелец — проверяя,
-- работает ли доставка, — и именно ему письмо не приходило. Копия
-- себе стоит ничего, а её отсутствие выглядит как поломка.
